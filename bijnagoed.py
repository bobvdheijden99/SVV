...
boundary[8][0] = -Pa * sinus - 2766                                                  # N, Sy(la) = 0

boundary[9][0] = - 20600 * cosin * (x2 - 0.5*xa)                     # Nm, My(la) = 0

boundary[10][0] = -20600 * (x2 - 0.5*xa) * cosin  - 3748.8            # Nm, Mz(la) = 0

boundary[11][0] = - Pa * ((0.5 * Ha * cosin) - SC * sinus)              # Nm, Mx(la) = 0
...
matrix[6][0] = ((- (1/EIzz) * (v(x2 - 0.15)[0]))*(sinus)) + ((1/GJ) * theta(x2 - 0.15)[0] * (((SC * np.sin( (25/180) * np.pi )) + (0.5 * Ha * cosin))))
...
matrix[9][0:3] = my(la)[0:3]
matrix[9][6] = my(la)[3]

matrix[10][0:3] = mz(la)[0:3]
matrix[10][6] = mz(la)[3]

matrix[11][0:3] = [SD,SD,SD]
matrix[11][6] = (0.5 * Ha * cosin - SC * sinus)
